...
const MultiPurchase = () => {
    ...
    const handleSubmit = async () => {
        ...
        const res = await fetch("https://godot-main-server.vercel.app/beforePurchase", {
            ...
            body: JSON.stringify({
                plan, email: userDetails.email, name: userDetails.name, refer: localStorage.getItem("refer"), wallet
            })
        })
    ...
    return (
        <div>
            <section style={{padding: "2rem 2rem 0 2rem"}} id="explore">
                ...
                <div className="navbar-slider showw" style={isOpen == 0 ? { visibility: 'visible' } : { visibility: 'hidden' }}>
                    <div className="purchase_card">
                        ...
                        <p className="disclaimer">Note: Please copy wallet address and send only BUSD to this wallet.</p>
                ...
                <div className="navbar-slider showw" style={isOpen == 2 ? { visibility: 'visible' } : { visibility: 'hidden' }}>
                    <div className="purchase_card">
                        ...
                        <p className="btn-grad" onClick={() => { push(`/dashboard`) }}>Continue</p>
                    </div>
                </div>
            ...
            {userPlans.includes("Platinum") ? <p className="btn-grad purchase" style={{ width: "20rem", height: "5rem", fontSize: "1.2rem", marginTop: "5rem" }} onClick={() => { index > -1 ? setisOpen(0) : alert("Please select a plan...") }}>Purchase</p> : <></>}
        </div>
    );
};

export default MultiPurchase;
